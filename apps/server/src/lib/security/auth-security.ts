...
export class AuthSecurityService {
  ...
  static validatePasswordStrength(password: string): {
    ...
    if (password.length > AUTH_SECURITY_CONFIG.MAX_PASSWORD_LENGTH) {
      violations.push(`Password must be no more than ${AUTH_SECURITY_CONFIG.MAX_PASSWORD_LENGTH} characters`);
    }
  ...
  static async evaluateLoginSecurity(
    ...
    if (additionalData?.location) {
      ...
      if (lastLocationData) {
        ...
        if (lastLocation.latitude && lastLocation.longitude && 
            additionalData.location.latitude && additionalData.location.longitude) {
      ...
      await cache.setex(lastLocationKey, 30 * 24 * 60 * 60, JSON.stringify(additionalData.location)); // 30 days
    }
    ...
    if (additionalData?.deviceFingerprint) {
      ...
      if (knownDevices) {
        ...
        await cache.setex(deviceKey, 90 * 24 * 60 * 60, JSON.stringify([additionalData.deviceFingerprint]));
      }
    }
  ...
  static async createSession(
    ...
    sessions = sessions.filter(s => s.isActive && (Date.now() - s.lastActivity) < AUTH_SECURITY_CONFIG.SESSION_TIMEOUT);
    ...
    await cache.setex(sessionKey, AUTH_SECURITY_CONFIG.SESSION_TIMEOUT / 1000, JSON.stringify(session));
  ...
  static async getUserSessions(userId: string): Promise<UserSession[]> {
    ...
    const activeSessions = sessions.filter(s => 
      s.isActive && (Date.now() - s.lastActivity) < AUTH_SECURITY_CONFIG.SESSION_TIMEOUT
    );
